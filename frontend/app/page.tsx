...
import { approveWork, archiveWork, clearSession, confirmOfflinePayment, createOrder, getCreatorDashboard, getCurrentUser, getEntitlements, getFunnel, getMarketplace, getMyOrders, getOfflinePaymentInfo, getPendingOfflinePayments, getPendingReviews, getWorkVersions, loginWithPassword, openOwnedSource, publishWork, registerWithPassword, rejectWork, releaseWorkVersion, savePaymentQr, simulatePaymentFailed, simulatePaymentSuccess, simulateRefund, submitPaymentProof, trackWorkViewed, updateWork, upgradeOwnedWork, uploadCoverImage, uploadWorkPackage, type AppUser, type CreatorDashboard, type Funnel, type MarketplaceItem, type MyOrder, type MyWork, type OfflinePaymentInfo, type OwnedWork, type PendingOfflinePayment, type ReviewWork, type WorkVersionHistory } from "./vibe-api";

const OFFICIAL_TOOLS_URL = "/?view=official-tools";

type View =
  | "home" | "map" | "explore" | "official-tools" | "wish" | "work" | "experience" | "entitlements" | "my-orders" | "mentor" | "profile"
...
export default function Home() {
  ...
  const [selected, setSelected] = useState({ method: "填写链接", title: "", description: "", tags: "网页, 来造", coverUrl: "", sourceUrl: "", priceYuan: "0", format: "网页", destination: "站内分享", access: "公开", pricing: "免费体验" });
  ...
  useEffect(()=>{
    if(new URLSearchParams(window.location.search).get("view")!=="official-tools")return;
    queueMicrotask(()=>{
  ...
  async function publishToMarketplace() {
    if (!selected.title.trim() || !selected.description.trim() || (selected.method === "填写链接" && !selected.sourceUrl.trim()) || (selected.method === "导入文件" && !workFile)) {
      flash("请先填写标题、简介，并提供网页链接或 ZIP 作品包");
      return;
    }
  ...
  return (
    <main className={`next-app view-${view}`}>
      ...
      {view === "explore" && <ExploreView go={go} flash={flash} marketplace={marketplace} onBuy={purchaseFromMarketplace} />}
      ...
      {view === "creator" && <CreatorView go={go} flash={flash} />}
...
function ExploreView({ go, flash, marketplace, onBuy }: { go:(v:View,l:string)=>void; flash:(s:string)=>void; marketplace:MarketplaceItem[]; onBuy:(item:MarketplaceItem)=>void }) { const [tab,setTab]=useState("为你推荐"); return <Page title="先找到一个，马上就能用" eyebrow="DISCOVER" intro=""><div className="explore-search"><span>⌕</span><input aria-label="搜索作品" placeholder="搜索工具、作品或问题"/><button onClick={() => flash("已整理 18 个相关结果")}>搜索</button></div><div className="scene-chips">{["旅行生活","内容创作","工作效率","图像创意","学习研究","健康管理"].map(x=><button key={x} onClick={()=>flash(`已切换到：${x}`)}>{x}</button>)}</div><div className="discovery-tools"><div className="content-heading"><div><h2>热门官方工具</h2></div><button className="secondary" onClick={()=>{window.location.href=OFFICIAL_TOOLS_URL}}>查看全部工具 →</button></div><div>{officialTools.slice(0,4).map((tool,i)=><button key={tool.name} onClick={()=>{window.location.href=OFFICIAL_TOOLS_URL}}><span className={`tool-mark tool-mark-${i}`}>{tool.icon}</span><b>{tool.name}</b><small>{tool.tag}</small><em>打开 →</em></button>)}</div></div><div className="explore-tabs">{["为你推荐","稳定工具","新鲜尝试","关注更新"].map(x=><button key={x} className={tab===x?"on":""} onClick={()=>setTab(x)}>{x}</button>)}<span>126 个创作者作品</span></div><button className="featured-work" onClick={()=>go("work","松弛感旅行规划器")}><div className="featured-art scene-1"/><div><span>编辑推荐</span><h2>松弛感旅行规划器</h2><div><b>免费体验</b><small>12.8k 有效使用 · 4.8 分</small><em>开始体验 →</em></div></div></button><div className="content-heading"><div><h2>{tab}</h2></div><div className="format-legend"><span>网页</span><span>Skill</span></div></div><div className="work-grid consumer-grid">{works.map((w,i) => <article className="work-card" key={w.id}><button className={`work-cover art-cover scene-${i+1}`} onClick={() => go("work", w.title)}><small>{w.type}</small><em>立即体验</em></button><div className="work-body"><div className="work-title"><h3>{w.title}</h3>{w.enterprise && <span className="verified">企业可采购</span>}</div><div className="trust-line"><span>✓ 已审核</span><span>{i===2?"允许试用":"可保存"}</span></div><div className="author"><span>{w.author.slice(0,1)}</span>{w.author}<i>✓</i></div><div className="work-meta"><b>{w.price}</b><span>{w.uses} 次使用</span></div></div></article>)}</div>{marketplace.length>0 && <><div className="content-heading"><div><h2>本地交易市场</h2><small>以下作品由 FastAPI + PostgreSQL 提供</small></div></div><div className="work-grid consumer-grid">{marketplace.map((item,i)=><article className="work-card" key={item.listing_id}><div className={`work-cover art-cover scene-${(i%4)+1}`} style={item.cover_url?{backgroundImage:`linear-gradient(rgba(8,18,40,.12),rgba(8,18,40,.18)),url(${item.cover_url})`,backgroundSize:"cover",backgroundPosition:"center"}:undefined}><small>{item.tags[0] || "作品"}</small><em>已上架</em></div><div className="work-body"><div className="work-title"><h3>{item.title}</h3><span className="verified">可交易</span></div><p>{item.description}</p><div className="author"><span>{item.creator_name.slice(0,1)}</span>{item.creator_name}<i>✓</i></div><div className="work-meta"><b>{item.price_cents === 0 ? "免费" : `¥${(item.price_cents / 100).toFixed(2)}`}</b><button className="secondary" onClick={()=>onBuy(item)}>获取作品 →</button></div></div></article>)}</div></>}<div className="not-found"><div><b>没有找到合适内容？</b></div><button onClick={()=>go("enterprise","定制合作")}>进入定制合作 →</button></div></Page> }
...
function CreatorView({ go, flash }:{go:(v:View,l:string)=>void;flash:(s:string)=>void}) {
  ...
  return <Page title="创作者中心" eyebrow="创作与经营" intro="">
    ...
    {error?<div className="empty-state"><b>{error}</b><span>请先完成手机号登录。</span></div>:<>
...
function CreatorWorkEditor({work,onClose,onSaved,flash}:{work:MyWork;onClose:()=>void;onSaved:()=>void;flash:(s:string)=>void}) {
  const [title,setTitle]=useState(work.title); const [description,setDescription]=useState(work.description); const [tags,setTags]=useState(work.tags); const [coverUrl,setCoverUrl]=useState(work.cover_url||""); const [priceYuan,setPriceYuan]=useState(String((work.price_cents||0)/100)); const [coverFile,setCoverFile]=useState<File|null>(null); const [saving,setSaving]=useState(false);
  const save=async()=>{const price=Number(priceYuan);if(!title.trim()||!Number.isFinite(price)||price<0){flash('请填写作品名称和正确价格');return;}try{setSaving(true);const uploaded=coverFile?await uploadCoverImage(coverFile):null;await updateWork(work.work_id,{title:title.trim(),description:description.trim(),tags:tags.split(/[,，]/).map(x=>x.trim()).filter(Boolean),coverUrl:uploaded?.source_url||coverUrl.trim(),priceCents:Math.round(price*100)});flash('已保存并重新提交审核');onSaved();}catch(e){flash(e instanceof Error?e.message:'保存失败');}finally{setSaving(false);}};
  return <div className="dialog-backdrop" role="presentation"><section className="auth-dialog" role="dialog" aria-modal="true" aria-label="编辑作品"><button className="dialog-close" onClick={onClose}>×</button><span className="dialog-kicker">EDIT WORK</span><h2>编辑作品</h2><p>修改后的作品会暂时从广场撤下，等待管理员重新审核。</p><label>作品标题<input value={title} onChange={e=>setTitle(e.target.value)} maxLength={120}/></label><label>作品简介<textarea value={description} onChange={e=>setDescription(e.target.value)} maxLength={10000}/></label><label>标签（用逗号分隔）<input value={tags} onChange={e=>setTags(e.target.value)} placeholder="AI, 网页工具"/></label><div className="two"><label>价格（元）<input type="number" min="0" step="0.01" value={priceYuan} onChange={e=>setPriceYuan(e.target.value)}/></label><label>封面图片（PNG/JPG/WebP，≤5MB）<input type="file" accept="image/png,image/jpeg,image/webp" onChange={e=>setCoverFile(e.target.files?.[0]||null)}/></label></div><label>或填写封面图片链接<input value={coverUrl} onChange={e=>setCoverUrl(e.target.value)} placeholder="https://…"/></label>{coverFile&&<small>将上传：{coverFile.name}</small>}<div className="dialog-actions"><button className="secondary" onClick={onClose}>取消</button><button className="primary" disabled={saving} onClick={save}>{saving?'保存中…':'保存并提交审核'}</button></div></section></div>;
}
...
function CreateView({ step,setStep,selected,choose,workFile,setWorkFile,coverFile,setCoverFile,go,flash }:{step:number;setStep:(n:number)=>void;selected:any;choose:(k:any,v:string)=>void;workFile:File|null;setWorkFile:(file:File|null)=>void;coverFile:File|null;setCoverFile:(file:File|null)=>void;go:(v:View,l:string)=>void;flash:(s:string)=>void}) {
  const sections = [
    ...
    <div key="r" className="choice-wrap"><h2>确认并提交</h2><p>站内发布或交付会先进入草稿箱，再由平台审核与分级。</p><div className="summary"><Summary label="作品来源" value={selected.method}/>{selected.method==="填写链接"&&<Summary label="访问链接" value={selected.sourceUrl||"尚未填写"}/>}<Summary label="作品形态" value={selected.format}/><Summary label="使用去向" value={selected.destination}/><Summary label="访问权限" value={selected.access}/><Summary label="收费" value={selected.pricing}/></div><div className="review-box"><span>平台预检</span><b>未发现明显风险</b><small>提交后将检查运行、版权、隐私、成本和内容质量</small></div></div>
  ];
...
function PublishSuccess({go,selected,onPublish}:{go:(v:View,l:string)=>void;selected:any;onPublish:()=>Promise<void>}) { const [publishing,setPublishing]=useState(false); const submit=async()=>{setPublishing(true);try{await onPublish();}finally{setPublishing(false);}}; return <Page title="作品已进入审核" eyebrow="SUBMITTED" intro="审核通过后会按你的设置进入站内分享；你仍可以在草稿箱撤回或修改。"><div className="success-panel"><Icon>✓</Icon><h2>「我的新作品」提交成功</h2><p>{selected.format} · {selected.destination} · {selected.access} · {selected.pricing}</p><div className="status-line"><span className="done">草稿完成</span><i/><span className="on">平台审核</span><i/><span>分类与分级</span><i/><span>正式发布</span></div><div className="success-actions"><button className="primary" disabled={publishing} onClick={submit}>{publishing ? "正在发布…" : "发布到本地交易市场"}</button><button className="secondary" onClick={()=>go("creator","创作者中心")}>返回创作者中心</button><button className="secondary" onClick={()=>go("platform","平台治理")}>查看平台如何审核</button></div></div></Page> }
