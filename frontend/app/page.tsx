...
import { approveWork, archiveWork, clearSession, confirmOfflinePayment, createOrder, deployStaticWork, getCreatorDashboard, getCurrentUser, getEntitlements, getFunnel, getLatestDataQuality, getMarketplace, getMyOrders, getOfflinePaymentInfo, getOperationsOverview, getPendingOfflinePayments, getPendingReviews, getWorkVersions, inspectViteBuild, loginWithPassword, openOwnedSource, publishWork, queueViteBuild, registerWithPassword, rejectWork, releaseWorkVersion, runDataQualityChecks, savePaymentQr, simulatePaymentFailed, simulatePaymentSuccess, simulateRefund, submitPaymentProof, trackWorkTried, trackWorkViewed, updateWork, upgradeOwnedWork, uploadCoverImage, uploadWorkPackage, type AppUser, type CreatorDashboard, type DataQualityItem, type Funnel, type MarketplaceItem, type MyOrder, type MyWork, type OfflinePaymentInfo, type OperationsOverview, type OwnedWork, type PendingOfflinePayment, type ReviewWork, type WorkVersionHistory } from "./vibe-api";
...
type View =
  ...
  | "platform" | "admin-login" | "admin-overview" | "admin-operations" | "admin-review" | "admin-reports"
...
const adminViews: View[] = ["admin-overview","admin-operations","admin-review","admin-reports","admin-contributors","admin-audit","admin-config"];
    ...
    <main className={`next-app view-${view}`}>
      <header className="topbar">
        <div className="shell topbar-inner">
          <button className="brand" onClick={() => jump("home", "首页")}><span className="brand-mark" aria-hidden="true">来</span>来造 Next <small>AI WORKS</small></button>
          <nav aria-label="主导航">
          ...
          <div className="top-actions">
            ...
            {currentUser ? <div style={{position:"relative"}}><button className="avatar system-avatar" title="账户菜单" aria-label={`${currentUser.display_name}的账户菜单`} aria-expanded={accountMenuOpen} onClick={() => setAccountMenuOpen(v=>!v)} />{accountMenuOpen&&<div role="menu" style={{position:"absolute",right:0,top:"calc(100% + 10px)",zIndex:100,width:210,padding:10,background:"#fff",border:"1px solid #dce3ef",boxShadow:"0 14px 32px rgba(18,35,70,.18)",color:"#17233d"}}><div style={{padding:"6px 8px 10px",borderBottom:"1px solid #edf0f5",marginBottom:6}}><b>{currentUser.display_name}</b><small style={{display:"block",color:"#73809a",marginTop:3}}>账号：{currentUser.username||"未设置"}</small></div><button className="text-btn wide" role="menuitem" onClick={()=>{setAccountMenuOpen(false);jump("creator","创作者中心");}}>创作者中心</button><button className="text-btn wide" role="menuitem" onClick={()=>{setAccountMenuOpen(false);jump("my-orders","我的订单");}}>我的订单</button><button className="text-btn wide" role="menuitem" onClick={()=>{setAccountMenuOpen(false);jump("entitlements","我的权益");}}>我的权益</button><button className="danger wide" role="menuitem" style={{marginTop:6}} onClick={()=>{clearSession();setCurrentUser(null);setAccountMenuOpen(false);flash("已退出登录");}}>退出登录</button></div>}</div> : <button className="ghost compact" onClick={() => setAuthOpen(true)}>登录 / 注册</button>}
          </div>
        </div>
      </header>
      ...
      {view === "home" && <HomeView go={go} marketplace={marketplace} onOpen={(item)=>{setSelectedMarketWork(item);go("market-detail",item.title);}} />}
      ...
      {view === "admin-operations" && canAccessAdmin && <AdminOperations flash={flash} />}
...
function HomeView({ go, marketplace, onOpen }: { go: (v: View, l: string) => void; marketplace: MarketplaceItem[]; onOpen: (item: MarketplaceItem) => void }) {
  const featured = marketplace.slice(0, 4);
  return <>
    <section className="next-home"><div className="shell">
      ...
      <div className="home-bento">
        <button className="bento-main" onClick={()=>featured[0]?onOpen(featured[0]):go("explore","作品广场")}><div className="bento-visual scene-1"/><div className="bento-copy"><h2>{featured[0]?.title||"等待第一个已审核作品"}</h2><footer><em>立即体验 →</em></footer></div></button>
        <button className="bento-idea" onClick={()=>go("mentor","提示词导师")}><UiIcon name="mentor" size="large"/><h2>导师 Agent</h2><b>生成外部提示词 →</b></button>
        <button className="bento-discover" onClick={()=>go("explore","作品广场")}><div><strong>{marketplace.length} 个已审核作品</strong></div><div className="mini-stack">{featured.slice(0,3).map((w,i)=><i className={`scene-${i+2}`} key={w.listing_id}/>)}</div><b>打开作品广场 →</b></button>
    ...
    <section className="home-library"><div className="shell"><div className="library-head"><div><span>真实上架数据</span><h2>先看结果，再选择工具</h2></div><button onClick={()=>go("explore","作品广场")}>查看全部作品 →</button></div><div className="library-grid">{featured.map((w,i)=><button key={w.listing_id} onClick={()=>onOpen(w)}><div className={`library-cover scene-${i+1}`}><span>网页</span></div><div><h3>{w.title}</h3><footer><span>{w.creator_name}</span><b>{w.price_cents===0?"免费":`¥${(w.price_cents/100).toFixed(2)}`}</b></footer></div></button>)}{!featured.length&&<p>暂时没有已审核上架作品。</p>}</div></div></section>
  ...
  return <Page title="作品广场" eyebrow="DISCOVER · TRY BEFORE YOU GET" intro="先免费体验作品，再决定是否获取完整版本。">
    ...
    <div className="work-grid consumer-grid">
      {marketplace.map((item,i)=><article className="work-card" key={item.listing_id}>
        ...
        <div className="work-body"><div className="work-title"><h3>{item.title}</h3><span className="verified">可体验</span></div><p>{item.description}</p><div className="author"><span className="creator-avatar" aria-label={`${item.creator_name}的默认头像`}/>{item.creator_name}<i>✓</i></div><div className="work-meta"><b>{item.price_cents===0?"免费":`¥${(item.price_cents/100).toFixed(2)}`}</b><button className="secondary" onClick={()=>onOpen(item)}>免费体验 →</button></div></div>
      </article>)}
    </div>
...
function CreatorView({ go, flash }:{go:(v:View,l:string)=>void;flash:(s:string)=>void}) {
  ...
  const inspectBuild=async(work:MyWork)=>{try{const result=await inspectViteBuild(work.work_id);flash(result.message);}catch(e){flash(e instanceof Error?e.message:'构建检查失败');}};
  const buildVite=async(work:MyWork)=>{try{const task=await queueViteBuild(work.work_id);flash(task.status==='queued'?'已进入隔离构建队列，稍后刷新查看体验版。':'该作品已有构建任务正在进行');refresh();}catch(e){flash(e instanceof Error?e.message:'提交构建失败');}};
  return <Page title="创作者中心" eyebrow="创作与经营" intro="">
    ...
    {error?<div className="empty-state"><b>{error}</b><span>请先登录账号后，再管理和发布作品。</span></div>:<>
      ...
      <section className="admin-card">{(data?.works||[]).map(work=><div className="admin-table-row" key={work.work_id}><b>{work.title}</b><span className={work.review_status==='rejected'?'status-red':work.review_status==='approved'?'status-green':'status-soft'}>{status(work.review_status)}</span><span>{work.price_cents===null?'未定价':work.price_cents===0?'免费':`¥${(work.price_cents/100).toFixed(2)}`}</span><span>{work.deployment_status==='ready'?'体验已部署':work.deployment_status==='failed'?`部署失败：${work.deployment_error}`:work.deployment_status==='queued'?'等待隔离构建':work.deployment_status==='building'?'正在隔离构建':work.review_note||'等待审核'}</span><span><button className="secondary" onClick={()=>setEditing(work)}>编辑</button><button className="secondary" onClick={()=>setReleasing(work)}>更新版本</button><button className="secondary" onClick={()=>inspectBuild(work)}>检查 Vite ZIP</button><button className="secondary" onClick={()=>buildVite(work)}>构建 Vite 体验版</button><button className="secondary" onClick={()=>deploy(work)}>{work.deployment_status==='ready'?'重新部署静态体验版':'部署静态 ZIP'}</button>{work.listing_status!=="archived"&&<button className="danger" onClick={()=>archive(work)}>下架</button>}</span></div>)}{data&&!data.works.length&&<p>尚未发布作品。</p>}</section>
...
function AdminNav({view,jump}:{view:View;jump:(v:View,l:string)=>void}) {
  const items:[View,string,UiIconName][]= [["admin-overview","数据总览","analysis"],["admin-operations","数据运营中心","wallet"],["admin-review","作品审核","testing"],["admin-reports","举报处理","security"],["admin-contributors","贡献者管理","profile"],["admin-audit","审计与归档","document"],["admin-config","平台配置","settings"]];
...
function AdminOperations({flash}:{flash:(s:string)=>void}) {
  const [overview,setOverview]=useState<OperationsOverview|null>(null);
  const [quality,setQuality]=useState<DataQualityItem[]>([]);
  const [running,setRunning]=useState(false);
  const refresh=()=>Promise.all([getOperationsOverview().then(setOverview),getLatestDataQuality().then(setQuality)]).catch((e:Error)=>flash(e.message));
  useEffect(()=>{void refresh();},[]);
  const run=async()=>{try{setRunning(true);setQuality(await runDataQualityChecks());flash("数据质量检查已完成");await getOperationsOverview().then(setOverview);}catch(e){flash(e instanceof Error?e.message:"检查失败");}finally{setRunning(false);}};
  const money=(value:number)=>`¥${(value/100).toFixed(2)}`;
  const metrics=overview?[[money(overview.gross_payment_cents),"账本上线后支付"],[money(overview.refund_cents),"账本上线后退款"],[money(overview.platform_net_revenue_cents),"平台净佣金"],[money(overview.creator_payable_cents),"创作者应付"],[String(overview.active_users_30d),"30 日活跃用户"],[String(overview.active_listings),"在售作品"],[String(overview.pending_versions),"待审核版本"],[`${(overview.refund_rate*100).toFixed(1)}%`,"退款率"]]:[];
  return <AdminPage title="数据运营中心" note={overview?`更新于 ${new Date(overview.generated_at).toLocaleString()}`:"正在读取权威数据"}>
    <div className="admin-metrics">{metrics.map(x=><article key={x[1]}><b>{x[0]}</b><span>{x[1]}</span></article>)}</div>
    <div className="admin-overview-grid">
      <section className="admin-card"><div className="admin-card-head"><h2>数据质量监控</h2><button className="primary" disabled={running} onClick={run}>{running?"检查中…":"立即检查"}</button></div>{quality.length===0?<p>尚未运行质量检查。</p>:quality.map(item=><div className="queue-row" key={item.check_name}><span>{item.message}</span><b className={item.status==="passed"?"status-green":"status-red"}>{item.status==="passed"?"正常":`${item.issue_count} 个异常`}</b><em>{new Date(item.checked_at).toLocaleString()}</em></div>)}</section>
      <section className="admin-card"><div className="admin-card-head"><h2>口径说明</h2><span>只读指标</span></div><p>支付、退款、佣金和创作者应付来自复式账本；用户活跃来自去重事件；作品与审核来自 PostgreSQL 业务表。</p><div className="queue-row"><span>累计用户</span><b>{overview?.users_total||0}</b><em>用户表</em></div><div className="queue-row"><span>已审核作品</span><b>{overview?.approved_works||0}</b><em>作品表</em></div><div className="queue-row"><span>发生支付订单</span><b>{overview?.paid_orders||0}</b><em>订单表</em></div></section>
    </div>
  </AdminPage>;
}
...
function AdminReview({flash}:{flash:(s:string)=>void}) {
  ...
  return <AdminPage title="作品审核" note={loading?'正在读取待审核作品':`待审 ${items.length} 件`}><div className="admin-tabs"><button className="on">待审核</button><button onClick={refresh}>刷新</button></div>{items.length===0&&!loading?<section className="admin-card"><h2>暂无待审核作品</h2><p>创作者提交作品后会自动出现在这里。</p></section>:items.map(item=><section className="review-card" key={item.id}><div><span className="status-soft">待人工审核</span><small>{item.tags || '未分类'} · {new Date(item.created_at).toLocaleString()}</small><h2>{item.title}</h2><p>{item.description || '未填写作品介绍'}</p>{item.cover_url&&<img src={item.cover_url} alt={`${item.title}封面`} style={{width:160,maxHeight:100,objectFit:"cover",borderRadius:10,border:"1px solid #dce3ef"}}/>}<div className="review-source-actions"><a className="secondary" href={item.trial_url||undefined} target="_blank" rel="noreferrer" aria-disabled={!item.trial_url}>{item.trial_url?'打开体验版 ↗':'体验版未部署'}</a><button className="secondary" onClick={()=>openOwnedSource(item.source_url).catch((e:Error)=>flash(e.message))}>{item.source_url.includes('/v1/files/')?'下载原始 ZIP':'打开作品来源 ↗'}</button></div><small>部署状态：{item.deployment_status||'未部署'}{item.deployment_error?` · ${item.deployment_error}`:''}</small></div><div className="review-actions"><button className="primary" disabled={!item.trial_url} title={!item.trial_url?'请先确认体验版已部署':undefined} onClick={()=>approve(item.id)}>通过</button><button className="danger" disabled={rejecting===item.id} onClick={()=>reject(item.id)}>{rejecting===item.id?'处理中…':'驳回'}</button></div><aside><b>审核提示</b><span>先打开体验版确认可运行</span><span>必要时下载 ZIP 检查内容</span><span>确认不含侵权或敏感内容</span><span>驳回时请写清修改原因</span></aside></section>)}<AdminOfflinePayments flash={flash}/></AdminPage>
}
