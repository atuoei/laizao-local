...
function MarketplaceDetailView({item,go,onBuy,flash}:{item:MarketplaceItem;go:(v:View,l:string)=>void;onBuy:(item:MarketplaceItem)=>void;flash:(s:string)=>void}) {
  const tryWork=()=>{ void trackWorkTried(item.work_id,item.listing_id); if(!item.trial_url){flash("该作品暂未配置体验链接");return;} window.open(item.trial_url,"_blank","noopener,noreferrer"); };
